...
@Path("fox/location/uris")
@RequestScoped
public class FoxServiceLocationUris {

	/**
	 * Returns location URIs as JSON array.
	 * 
	 * e.g.
	 * http://localhost:9080/metadata/fox/location/uris?text=Paderborn%20und%20Bad%20Oeynhausen%20sind%20in%20NRW&lang=de
	 */
	@GET
	...
	public Response getLocationUris(@Context UriInfo info) {
		try {
			...
			List<String> locationUris = new Extraction().extractLocationUris(fox.getTurtle(text, lang));
			return Response.ok(new JSONArray(locationUris).toString()).build();
		} catch (Throwable t) {
